...
随着 **ChatGPT** 横空出世并取代 **Bert** 成为新一代自然语言处理的主流，全球范围内的大模型（Large Language Model，LLM）竞争进入了一个前所未有的“百模大战”时代。

大模型强大的通用性和推理能力无可争议，但在某些实时、动态或专门领域的问题上，往往显得力不从心。

为此，给大模型“接上”一些更贴近真实世界信息或提供专门功能的工具，能在一定程度上弥补此缺陷，让大模型获得更细粒度、更接近现实需求的能力。

**TinyAgent** 便是这样一个尝试：基于论文 [*ReAct: Synergizing Reasoning and Acting in Language Models*](http://arxiv.org/abs/2210.03629) 的思路，手动搭建了一个极简的 Agent 系统，使得大模型能调用外部工具来完成更复杂的任务。

## 3.1ReAct 方法概述

在自然语言处理领域，“ReAct”(**Re**ason and **Act**) 提出了一种将推理（Reasoning）与行动（Acting）结合的思路。

传统大模型往往缺乏与外部世界交互的接口，只能通过内部知识或语言生成能力来回答用户问题；

而 ReAct 则允许模型在回答用户问题的过程中，调用“工具”执行操作，并将操作结果再次反馈给模型进行后续推理。

简单来说，这是一种让大模型在回答问题的过程中动态地“先思考，再行动”的范式，通常分为以下循环步骤：

1. **Thought（思考）**：语言模型会从对话或问题中提炼出必要的线索，判断是否需要使用工具。
2. **Action（行动）**：若需要，可调用外部工具（如搜索引擎、数据库查询、计算模块等）。
3. **Observation（观察）**：模型获取工具返回的信息。
4. **Final Answer（最终回答）**：模型结合外部信息与自身知识，对问题给出最终回答。

这样一来，模型不再是“闭门造车”，而是可以通过外部接口动态获取信息或执行计算，从而在很多情形下获得比纯语言模型更准确、更实时的回答。

## 3.2TinyAgent：一个最小化的 ReAct 结构示例

本项目 **TinyAgent** 的目标是用极简的代码和思路，将 ReAct 的原理落地，帮助开发者更加直观地理解和掌握 Agent 的构建方式。

项目目前以 `React` 范式为基础，后续在会尝试将其改造为 `SOP`（Step-by-Step Orchestrator Process）结构。

### 3.2.1构造大模型

TinyAgent 需要一个基础大模型来进行语言推理。

使用了 **InternLM2** 作为对话模型，

加载方式采用了 `transformers` 包的常规写法，包括：

- 定义 `BaseModel` 基类，封装加载模型和对话接口的方法；
- 基于 `BaseModel` 派生 `InternLM2Chat` 类，按需求重载 `chat` 方法和 `load_model` 方法；
- 通过 `InternLM2Chat` 初始化并调用模型的聊天功能。

利用 `AutoTokenizer`、`AutoModelForCausalLM` 来加载 `InternLM2-chat` 模型并进行推理。

若希望在生产环境中得到更稳定的推理效果，可使用容量更大的 `InternLM2-20B-chat` 版本。

### 3.2.2 构造工具（Tools）

要让大模型能够“行动”，必须提供可以调用的工具。

此示例的工具以 **Google 搜索** 为例，通过 `serper` 平台申请 API Key，即可在 Python 中借助 HTTP 请求完成实际的搜索查询。

工具的关键点包括：

- 在 `tools.py` 文件中定义 `Tools` 类，并在 `_tools()` 方法里描述每个工具的参数和功能说明（供系统提示词使用）。
- 编写相应的工具调用函数，如 `google_search`，在收到搜索关键词后执行实际搜索并返回结果。

有了工具描述以及对应的实现，就能在系统提示（system prompt）中告诉模型如何“调用”此工具、工具需要的参数结构以及工具能提供的功能。

### 3.2.3. 构造 Agent

**Agent** 是整个系统的核心，它基于 ReAct 的模式来“驾驭”大模型。主要工作包括：

1. **构造 `system_prompt`**：在系统提示中告诉大模型有哪些工具可用、如何调用这些工具，以及期望它生成什么格式的输出（如 `Thought`, `Action`, `Action Input`, `Observation`, `Final Answer` 等）。
2. **解析模型输出**：在第一次模型推理时，大模型会返回 `Action` 和 `Action Input`，Agent 需要将这部分信息解析出来，然后调用相应的工具，并将工具的返回值作为新的“Observation”注入给模型。
3. **二次推理与最终回答**：得到工具的返回后，再次调用大模型，让它综合考虑“Observation”和用户问题，最终给出 `Final Answer`。

`Agent` 类的核心方法 `text_completion` 便体现了上述逻辑：

- 首先构造初始对话，包括 `system_prompt` 和用户输入；
- 让大模型思考并返回要调用的工具和参数；
- 解析后调用相应工具并将结果封装成 “Observation”；
- 最后再次将对话上下文交给模型，获取最终回答。

### 3.2.4. 运行 Agent

在完成上述步骤后，即可编写简单的测试脚本来验证 Agent 的工作流。例如，向 Agent 提问“周杰伦是哪一年出生的？”，如果大模型决定调用 `google_search` 工具，工具查询到结果后再由大模型合并信息，最终返回正确答案。示例中展示了多轮对话的具体过程和结果：
...
都可以看到大模型根据工具返回的信息进行“思考”与“回答”的过程。

## 3.3后续展望与总结

TinyAgent 展示了一个极简的 ReAct 思路示例，从中可以看到：

1. **大模型 + 工具** 的协同能力：通过在系统提示中向模型明确描述可用工具的接口和功能，模型可实时决定何时调用工具，并根据搜索结果或其它计算结果给出更加准确的回答。
2. **实现方式灵活**：项目目前使用 `InternLM2` 模型并集成 Google 搜索作为工具，后续可根据需要添加更多外部工具，如计算引擎、数据库接口、地图/GIS 分析、专业知识库等。
3. **结构可扩展**：TinyAgent 未来计划将 `React` 结构改造成 `SOP`（Step-by-Step Orchestrator Process），进一步提升可控性和可扩展性，让大模型在多步推理、调用多个工具时更有逻辑性与层次性。

这类 Agent 化的大模型应用，使 LLM 在特定场景下的能力显著增强，有助于解决许多实时、跨学科与深层推理的问题，也为大模型赋能真实业务场景提供了更好的接口和框架。
